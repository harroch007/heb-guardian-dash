...
export const HomePendingTimeRequests = ({ childrenData }: Props) => {
  ...
  return (
    <Card className="border-primary/30 bg-primary/5">
      <CardContent className="py-3 px-4 space-y-2">
        ...
        {requests.map((req) => {
          const childName = nameById.get(req.child_id) || "";
          return (
            <div
              key={req.id}
            ...
            >
              <div className="flex-1 min-w-0">
                <p className="text-sm text-foreground truncate">
                  ...
                  {req.reason ? ` · ${req.reason}` : " · ללא סיבה"}
                </p>
                <p className="text-[11px] text-muted-foreground">
                  {timeAgo(req.created_at)} · בקשה ל-{req.requested_minutes} דק׳
                </p>
              </div>
