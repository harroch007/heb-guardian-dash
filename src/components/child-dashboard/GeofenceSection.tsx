...
function PlaceCard({
  ...
  const handleUseDevice = () => {
    if (deviceLat != null && deviceLng != null) {
      setLat(String(deviceLat));
      setLng(String(deviceLng));
      setEditing(true);
    }
  };
  ...
  const handleSave = () => {
    ...
    onSave(latN, lngN, undefined, place?.radius_meters ?? defaultRadius);
    setEditing(false);
    setLat("");
    setLng("");
  };
    ...
    <div className="border border-border rounded-lg p-3 space-y-2">
      ...
      {place && !editing && (
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <Select
              value={String(place.radius_meters)}
              onValueChange={(v) => onUpdateRadius(Number(v))}
              dir="rtl"
            >
          ...
          <div className="flex gap-2">
            <Button variant="outline" size="sm" className="text-xs flex-1" onClick={handleStartEdit}>
              עדכן מיקום
            </Button>
      ...
      {editing && (
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            <Button size="sm" className="text-xs flex-1" onClick={handleSave} disabled={saving || !lat || !lng}>
              {saving ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : "שמור"}
            </Button>
            <Button variant="ghost" size="sm" className="text-xs" onClick={() => { setEditing(false); setLat(""); setLng(""); }}>
              ביטול
            </Button>
          </div>
        </div>
...
export function GeofenceSection({ childId, deviceLatitude, deviceLongitude, deviceAddress }: GeofenceSectionProps) {
  const { places, settings, loading, saving, getPlace, upsertPlace, updateRadius, updateSettings, deletePlace } = useChildPlaces(childId);
      ...
      <CardContent className="p-4 space-y-4">
        ...
        <div className="space-y-3">
          <PlaceCard
            ...
            onUpdateRadius={(r) => updateRadius("HOME", r)}
            onDelete={() => deletePlace("HOME")}
          />
          <PlaceCard
            type="SCHOOL"
            label="בית ספר"
            place={getPlace("SCHOOL")}
            saving={saving}
            deviceLat={deviceLatitude}
            deviceLng={deviceLongitude}
            deviceAddress={deviceAddress}
            onSave={(lat, lng, label, radius) => upsertPlace("SCHOOL", { latitude: lat, longitude: lng, label, radius_meters: radius })}
            onUpdateRadius={(r) => updateRadius("SCHOOL", r)}
            onDelete={() => deletePlace("SCHOOL")}
          />
        </div>
        ...
        <div className="border-t border-border pt-3 space-y-3">
          ...
          <div className="flex items-center justify-between">
            ...
            <div dir="ltr">
              <Switch
                checked={settings.home_exit_alert_enabled}
                onCheckedChange={(v) => updateSettings({ home_exit_alert_enabled: v })}
              />
            </div>
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <div dir="ltr">
              <Switch
                checked={settings.school_exit_alert_enabled}
                onCheckedChange={(v) => updateSettings({ school_exit_alert_enabled: v })}
              />
            </div>
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <Select
              value={String(settings.exit_debounce_seconds)}
              onValueChange={(v) => updateSettings({ exit_debounce_seconds: Number(v) })}
              dir="rtl"
            >
