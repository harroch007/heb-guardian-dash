...
import { optionalExpertReview } from "./_shared/incident_review.ts";
import {
  ageLookupTimeoutMs,
...
import { assertEphemeralContextFresh } from "./_shared/incident_ephemeral_deadline.ts";
import { AnalysisDeadlineError } from "./_shared/incident_deadline.ts";
import {
  isUuid,
  requireDevice,
  requiredString,
...
Deno.serve(async (request) => {
  try {
    ...
    if (body.operation === "get_assessment_receipt") {
      ...
      const receipt = await getThreeGateReceipt(
        ...
        async (caseId, assessmentId, sequence) => {
          const { data, error } = await client.rpc(
            "v2_get_three_gate_assessment_receipt_service",
            {
              target_device_id: device.deviceId,
              target_case_id: caseId,
              target_assessment_id: assessmentId,
              target_assessment_seq: sequence,
            },
          );
          if (error) throw error;
          const result = Array.isArray(data) ? data[0] : undefined;
          if (result === undefined) {
            throw new Error("missing_assessment_receipt_result");
          }
          return result;
        },
      );
      if (receipt.receipt_state === "completed") {
        receipt.expert_review = await optionalExpertReview(async () => {
          const { data, error } = await client.rpc(
            "v2_get_expert_review_service",
            {
              target_device_id: device.deviceId,
              target_case_id: body.case_id,
              target_assessment_id: body.assessment_id,
              target_assessment_seq: body.assessment_seq,
            },
          ).abortSignal(AbortSignal.timeout(1_000));
          if (error) throw error;
          return data;
        }, Number(receipt.message_count));
      }
    ...
    if (
      privacyContractVersion !== EPHEMERAL_PRIVACY_CONTRACT_VERSION
    ) {
      return await processStoredIncident(incidentInput);
    }
    return await processEphemeralExpertIncident(incidentInput);
  } catch (error) {
...
async function processStoredIncident(
  input: IncidentInput,
): Promise<Response> {
  const { data, error } = await input.client.rpc(
    "v2_submit_safety_incident_service",
    {
      ...
      target_privacy_contract_version: input.privacyContractVersion,
      target_privacy_identity_version: input.privacyIdentityVersion,
      target_encrypted_payload_base64: input.encryptedPayload,
      target_encryption_algorithm: input.encryptionAlgorithm,
  ...
  const result = data?.[0];
  if (result === undefined) throw new Error("missing_incident_result");
...
async function processEphemeralExpertIncident(
  ...
  try {
    assertEphemeralContextFresh(claim.context_expires_at_canonical);
    ...
    const fullFifo = await validateFullFifo(
      context,
      String(input.body.client_incident_id),
    );
    if (
      fullFifo !== undefined && (
        fullFifo.snapshot.cutoff_at_ms > Date.now() + 600_000 ||
        fullFifo.snapshot.cutoff_at_ms >=
          Date.parse(claim.context_expires_at_canonical)
      )
    ) throw new FullFifoError("invalid_full_fifo_snapshot_time");

    // Server-owned age metadata is added to the model view only, after validating
    // the original payload. Cached receipts never call this lookup or inference.
    const modelContextForInference = () =>
      enrichExpertContextWithAge(
        context,
        input.device,
        async (birthYear, calendarYear) => {
          const timeoutMs = ageLookupTimeoutMs(
            Date.parse(claim.context_expires_at_canonical),
          );
          if (timeoutMs === 0) return null;
          const { data, error } = await input.client.rpc("v2_child_age_band", {
            target_birth_year: birthYear,
            target_year: calendarYear,
    ...
    const beginArguments = {
      ...
      target_occurred_at: input.body.occurred_at,
      target_model_contract_version: input.contractVersion,
      target_privacy_contract_version: EPHEMERAL_PRIVACY_CONTRACT_VERSION,
      target_privacy_identity_version: input.privacyIdentityVersion,
      target_key_version: input.keyVersion,
      target_message_count: input.messageCount,
      target_context_expires_at: input.body.context_expires_at,
    ...
    const { data: begun, error: beginError } = await input.client.rpc(
      ...
      fullFifo === undefined ? beginArguments : {
        target_device_id: input.device.deviceId,
        target_header: beginArguments,
        target_metadata: fullFifoDatabaseMetadata(fullFifo),
        target_conversation_ref: context.conversation_ref,
      },
    );
    ...
    if (fullFifo !== undefined) {
      ...
      const response = await runThreeGateAssessment(begin, fullFifo, {
        ...
        finalize: async (modelResult) => {
          assertEphemeralContextFresh(claim.context_expires_at_canonical);
          const { data, error } = await input.client.rpc(
            "v2_finalize_three_gate_assessment_review_service",
            {
              target_incident_id: begin.incident_id,
              target_lease_token: begin.lease_token,
              target_analysis: modelResult.analysis,
              target_model_version: modelResult.modelVersion,
              target_prompt_version: EXPERT_PROMPT_VERSION,
              target_explanation: modelResult.explanation ?? null,
              target_explanation_status: modelResult.explanation_status ??
                "NOT_PROVIDED",
            },
          );
          if (error) throw error;
          const result = Array.isArray(data) ? data[0] : undefined;
          if (result === undefined) {
            throw new Error("missing_three_gate_finalize_result");
          }
          return result;
        },
      });
      leaseToken = undefined;
      response.expert_review = await optionalExpertReview(async () => {
        const { data, error } = await input.client.rpc(
          "v2_get_expert_review_service",
          {
            target_device_id: input.device.deviceId,
            target_case_id: fullFifo.case_id,
            target_assessment_id: fullFifo.assessment_id,
            target_assessment_seq: fullFifo.assessment_seq,
          },
        ).abortSignal(AbortSignal.timeout(1_000));
        if (error) throw error;
        return data;
      }, fullFifo.snapshot.message_count);
      return jsonResponse(response.created ? 201 : 200, response);
    }
    if (begin.analysis_state === "completed") {
      return jsonResponse(200, {
        incident_id: begin.parent_incident_id ?? incidentId,
        created: false,
    ...
    const modelContext = await modelContextForInference();
    const contextDeadlineMs = assertEphemeralContextFresh(
      claim.context_expires_at_canonical,
    );
    ...
    if (error instanceof FullFifoError) throw new HttpError(422, error.code);
