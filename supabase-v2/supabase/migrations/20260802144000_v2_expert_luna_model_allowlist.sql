-- Admit the current low-latency expert model without invalidating analyses
-- persisted by the two previously approved GPT-5.4 Nano identifiers.
--
-- The V3 finalizer owns this allowlist because it is the capability-protected
-- persistence boundary. Keep the stored model family aligned with the exact
-- provider model version instead of labeling Luna results as GPT-5.4 Nano.
...
alter function public.v2_finalize_incident_analysis_service(
    text,
    uuid,
    text,
    uuid,
