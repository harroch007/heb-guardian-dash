...
revoke execute on function public.v2_create_guardian_child(
    uuid,
    uuid,
    text,
    smallint,
...
comment on function public.v2_create_guardian_child(
    uuid,
    uuid,
    text,
    smallint,
    ...
    'Dormant compatibility signature. Guardian PWA uses the gender-aware V2 signature.';
...
begin
    ...
    if target_child_id is null
       ...
       or target_gender not in ('male', 'female', 'other') then
    ...
    if child_row.id is null
       ...
       or child_row.birth_year is distinct from target_birth_year
       or child_row.gender <> target_gender then
