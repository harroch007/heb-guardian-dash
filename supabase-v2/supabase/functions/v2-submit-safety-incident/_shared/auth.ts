...
export interface DeviceIdentity {
  deviceId: string;
  childId: string;
  /** Server-only registration metadata. Never serialize this identity to a model. */
  trustedBirthYear?: number | null;
}
...
export async function requireDevice(
  ...
  const { data: child, error: childError } = await client
    .from("v2_children")
    .select("id, family_id, status, birth_year")
  ...
  return {
    ...
    trustedBirthYear:
      typeof child.birth_year === "number" && Number.isInteger(child.birth_year)
        ? child.birth_year
        : null,
  };
}
