...
-- Must drop first to change return type from void to jsonb
DROP FUNCTION IF EXISTS public.update_device_status(text, integer, double precision, double precision, text, text);
...
BEGIN
  ...
  -- Priority 1: Issur Melacha (Shabbat/Yom Tov)
  IF EXISTS (
