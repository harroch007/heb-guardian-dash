...
BEGIN
    ...
    IF v_child_id IS NOT NULL THEN
        ...
        IF v_shabbat_row IS NOT NULL THEN
            v_start_epoch := extract(epoch from (v_shabbat_row.friday_date::date + v_shabbat_row.candle_lighting) AT TIME ZONE 'Asia/Jerusalem');
            v_end_epoch   := extract(epoch from ((v_shabbat_row.friday_date::date + 1) + v_shabbat_row.havdalah) AT TIME ZONE 'Asia/Jerusalem');
    ...
    -- Issur melacha windows
    IF v_child_id IS NOT NULL THEN
        SELECT COALESCE(jsonb_agg(
          jsonb_build_object(
            'id', iw.id,
            'event_type', iw.event_type,
            'event_name', iw.event_name,
            'start_at', iw.start_at,
            'end_at', iw.end_at,
            'start_epoch', extract(epoch from iw.start_at),
            'end_epoch', extract(epoch from iw.end_at)
          )
        ), '[]'::jsonb)
    ...
    IF v_child_id IS NOT NULL THEN
        ...
        v_settings := v_settings || jsonb_build_object(
            'geofence_places', v_geofence_places,
            'manual_geofence_places', v_manual_geofence_places,
            'geofence_settings', CASE
                WHEN v_geofence_settings IS NOT NULL THEN jsonb_build_object(
    ...
    -- Time request updates for device catch-up
    IF v_child_id IS NOT NULL THEN
        SELECT COALESCE(jsonb_agg(
          jsonb_build_object(
            'request_id', ter.id,
            'status', ter.status,
            'approved_minutes', ter.approved_minutes,
            'responded_at', ter.responded_at
          ) ORDER BY ter.responded_at DESC
        ), '[]'::jsonb)
        INTO v_time_request_updates
        FROM time_extension_requests ter
        WHERE ter.child_id = v_child_id
          AND ter.status IN ('approved', 'denied')
        LIMIT 20;
