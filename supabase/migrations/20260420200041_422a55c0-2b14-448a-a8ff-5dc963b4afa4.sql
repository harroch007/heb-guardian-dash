...
BEGIN
    ...
    -- ============ CRITICAL CORE: app_policies + blocked_apps ============
    -- This block must NEVER be wrapped in EXCEPTION — if it fails we want to know.
    IF v_child_id IS NOT NULL THEN
    ...
    -- ============ CRITICAL CORE: daily limit + bonus ============
    IF v_child_id IS NOT NULL THEN
    ...
    -- ============ CRITICAL CORE: schedules (school / sleep / shabbat rules) ============
    IF v_child_id IS NOT NULL THEN
        SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    -- ============ EXTENDED (NON-CRITICAL): reward bank ============
    IF v_child_id IS NOT NULL THEN
        BEGIN
        ...
        EXCEPTION WHEN OTHERS THEN
            v_extended_errors := v_extended_errors || jsonb_build_object('reward_bank', SQLERRM);
        END;
    END IF;

    -- ============ EXTENDED (NON-CRITICAL): next_shabbat ============
    IF v_child_id IS NOT NULL THEN
        BEGIN
            ...
            END IF;
        EXCEPTION WHEN OTHERS THEN
            v_extended_errors := v_extended_errors || jsonb_build_object('next_shabbat', SQLERRM);
        END;
    END IF;

    -- ============ EXTENDED (NON-CRITICAL): next_issur + issur_windows ============
    IF v_child_id IS NOT NULL THEN
        BEGIN
        ...
        EXCEPTION WHEN OTHERS THEN
            v_extended_errors := v_extended_errors || jsonb_build_object('issur_windows', SQLERRM);
            v_settings := v_settings || jsonb_build_object('issur_windows', '[]'::jsonb);
        END;
    END IF;

    -- ============ EXTENDED (NON-CRITICAL): geofence_places + geofence_settings ============
    IF v_child_id IS NOT NULL THEN
        BEGIN
            SELECT COALESCE(jsonb_agg(jsonb_build_object(
            ...
            END IF;
        EXCEPTION WHEN OTHERS THEN
            v_extended_errors := v_extended_errors || jsonb_build_object('geofence_places', SQLERRM);
            v_settings := v_settings || jsonb_build_object('geofence_places', '[]'::jsonb);
        END;
    END IF;

    -- ============ EXTENDED (NON-CRITICAL): manual_geofence_places ============
    IF v_child_id IS NOT NULL THEN
        BEGIN
            SELECT COALESCE(jsonb_agg(jsonb_build_object(
        ...
        EXCEPTION WHEN OTHERS THEN
            v_extended_errors := v_extended_errors || jsonb_build_object('manual_geofence_places', SQLERRM);
            v_settings := v_settings || jsonb_build_object('manual_geofence_places', '[]'::jsonb);
        END;
    END IF;

    -- ============ EXTENDED (NON-CRITICAL): time_request_updates ============
    IF v_child_id IS NOT NULL THEN
        BEGIN
            SELECT COALESCE(jsonb_agg(jsonb_build_object(
        ...
        EXCEPTION WHEN OTHERS THEN
            v_extended_errors := v_extended_errors || jsonb_build_object('time_request_updates', SQLERRM);
            v_settings := v_settings || jsonb_build_object('time_request_updates', '[]'::jsonb);
        END;
    END IF;

    -- Surface non-fatal errors so we can debug without breaking enforcement
    IF v_extended_errors <> '{}'::jsonb THEN
        v_settings := v_settings || jsonb_build_object('_extended_errors', v_extended_errors);
    END IF;
...
-- Smoke test: must return JSON with app_policies; raises if RPC fails
DO $$
DECLARE
    v_result jsonb;
    v_policies jsonb;
BEGIN
    v_result := public.get_device_settings('9d5a9132b033a86b');
    IF v_result IS NULL THEN
        RAISE EXCEPTION 'get_device_settings returned NULL';
    END IF;
    v_policies := v_result -> 'app_policies';
    IF v_policies IS NULL THEN
        RAISE EXCEPTION 'get_device_settings did not return app_policies. Result: %', v_result;
    END IF;
    RAISE NOTICE 'Smoke test OK. app_policies count: %, extended_errors: %',
        jsonb_array_length(v_policies), v_result -> '_extended_errors';
END $$;