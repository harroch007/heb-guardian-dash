...
-- Drop all dependent views and recreate them
...
-- 1. Recreate parent_alerts_effective with notify+review filtered by threshold
...
WHERE (
    ...
    AND a.ai_verdict IN ('notify', 'review')
...
OR (
    a.alert_type = 'positive'
    AND a.is_processed = true
);

-- 2. Recreate parent_daily_report
CREATE VIEW public.parent_daily_report AS
WITH base AS (
    ...
    SELECT pae.device_id, (pae.analyzed_at AT TIME ZONE 'UTC')::date AS day, count(*) AS cnt_notify_effective
...
-- 3. Recreate parent_daily_report_for_parent (uses the DB function, not a view - but it was a view)
-- Actually it's referenced as a view, let me recreate it using the same function-based approach
-- Check: the RPC function get_parent_daily_report_for_parent already exists, so this view
-- just needs to wrap parent_daily_report with auth.uid() filter
CREATE VIEW public.parent_daily_report_for_parent AS
SELECT pdr.*
FROM parent_daily_report pdr
JOIN children c ON c.id = pdr.child_id
WHERE c.parent_id = auth.uid();

-- 4. Recreate parent_home_snapshot
CREATE VIEW public.parent_home_snapshot AS
WITH today AS (
    ...
    FROM app_usage au JOIN today t ON t.day = au.usage_date GROUP BY au.device_id
), top_apps AS (
    SELECT au.device_id,
    ...
    FROM daily_chat_stats dcs JOIN today t ON t.day = dcs.stat_date GROUP BY dcs.device_id
), notify_effective AS (
