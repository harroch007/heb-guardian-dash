...
-- 1. Add approved_minutes column (idempotent check)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'time_extension_requests' AND column_name = 'approved_minutes'
  ) THEN
    ALTER TABLE public.time_extension_requests ADD COLUMN approved_minutes integer;
  END IF;
END $$;

-- 2. Backfill existing approved rows from bonus_time_grants
UPDATE time_extension_requests ter
SET approved_minutes = btg.bonus_minutes
FROM bonus_time_grants btg
WHERE btg.child_id = ter.child_id
  AND btg.granted_by IS NOT NULL
  AND ter.status = 'approved'
  AND ter.responded_at IS NOT NULL
  AND ter.approved_minutes IS NULL
  AND btg.created_at >= ter.created_at
  AND btg.created_at <= ter.responded_at + interval '5 seconds';

-- 3. Drop and recreate respond_time_request
DROP FUNCTION IF EXISTS public.respond_time_request(uuid, boolean, integer);

CREATE OR REPLACE FUNCTION public.respond_time_request(
  p_request_id uuid,
  p_approved boolean,
  p_minutes integer
)
...
BEGIN
  ...
  SET status = CASE WHEN p_approved THEN 'approved' ELSE 'rejected' END,
      responded_at = now(),
      approved_minutes = CASE WHEN p_approved THEN p_minutes ELSE null END
  WHERE id = p_request_id;
...
-- 4. Update get_device_settings to use approved_minutes column
