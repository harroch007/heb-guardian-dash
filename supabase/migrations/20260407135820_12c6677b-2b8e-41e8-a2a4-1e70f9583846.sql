...
DECLARE
    ...
    v_jwt_role text;
BEGIN
    -- Phase 4A: Fail-closed authorization gate (device-only)
    -- Step 1: Must be an authenticated caller (not anon, not service_role)
    IF auth.role() IS DISTINCT FROM 'authenticated' THEN
        RETURN jsonb_build_object('error', 'UNAUTHORIZED', 'success', false);
    END IF;

    -- Step 2: Must have app_metadata.role = 'device'
    v_jwt_role := (auth.jwt() -> 'app_metadata' ->> 'role');
    IF v_jwt_role IS DISTINCT FROM 'device' THEN
        RETURN jsonb_build_object('error', 'UNAUTHORIZED', 'success', false);
    END IF;

    -- Step 3: JWT device_id must match the requested p_device_id
