...
-- 1. Extend place_type CHECK to allow MANUAL
...
-- 3. Add CHECK for schedule_mode values
ALTER TABLE child_places ADD CONSTRAINT child_places_schedule_mode_check
  CHECK (schedule_mode IN ('ALWAYS', 'SCHEDULED'));

-- 4. Fix unique index: only constrain HOME/SCHOOL, allow multiple MANUAL
...
-- 5. Validation trigger for MANUAL + SCHEDULED rules
CREATE OR REPLACE FUNCTION validate_child_place()
RETURNS TRIGGER
LANGUAGE plpgsql
SET search_path = public
AS $$
BEGIN
  -- MANUAL places must have a non-empty label
  IF NEW.place_type = 'MANUAL' AND (NEW.label IS NULL OR trim(NEW.label) = '') THEN
    RAISE EXCEPTION 'MANUAL places must have a non-empty label';
  END IF;

  -- SCHEDULED mode requires valid schedule fields
  IF NEW.schedule_mode = 'SCHEDULED' THEN
    IF NEW.days_of_week IS NULL OR array_length(NEW.days_of_week, 1) IS NULL THEN
      RAISE EXCEPTION 'SCHEDULED places must have at least one day in days_of_week';
    END IF;
    IF NEW.start_time IS NULL OR NEW.end_time IS NULL THEN
      RAISE EXCEPTION 'SCHEDULED places must have start_time and end_time';
    END IF;
  END IF;
...
CREATE TRIGGER trg_validate_child_place
  BEFORE INSERT OR UPDATE ON child_places
  FOR EACH ROW
  EXECUTE FUNCTION validate_child_place();

-- 6. Update get_device_settings: filter geofence_places to HOME/SCHOOL, add manual_geofence_places
