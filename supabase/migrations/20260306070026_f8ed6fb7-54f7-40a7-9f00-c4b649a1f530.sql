...
-- 1. Drop the broken create_permission_alert RPC
DROP FUNCTION IF EXISTS public.create_permission_alert(text, text, text);

-- 2. Create trigger function for heartbeat permission change detection
...
-- 3. Create the trigger on device_heartbeats_raw
DROP TRIGGER IF EXISTS trg_heartbeat_permission_check ON public.device_heartbeats_raw;
CREATE TRIGGER trg_heartbeat_permission_check
  AFTER INSERT ON public.device_heartbeats_raw
  FOR EACH ROW
  EXECUTE FUNCTION public.on_heartbeat_insert();
